...
>((props: ScopedProps<SelectItemAlignedPositionProps>, forwardedRef) => {
  ...
  const [content, setContent] = React.useState<SelectItemAlignedPositionElement | null>(
    null,
  );
  const composedRefs = useComposedRefs(forwardedRef, (node) => setContent(node));
  const getItems = useCollection(__scopeSelect);
...
const SelectItem = React.forwardRef<SelectItemElement, SelectItemProps>(
  (props: ScopedProps<SelectItemProps>, forwardedRef) => {
    ...
    if (value === "") {
      throw new Error(
        "A <Select.Item /> must have a value prop that is not an empty string. This because the Select can be set to string clear selection and show placeholder.",
      );
    }
...
const SelectItemText = React.forwardRef<SelectItemTextElement, SelectItemTextProps>(
  (props: ScopedProps<SelectItemTextProps>, forwardedRef) => {
    ...
    const [itemTextNode, setItemTextNode] = React.useState<SelectItemTextElement | null>(
      null,
    );
...
>((props: ScopedProps<SelectScrollUpButtonProps>, forwardedRef) => {
  ...
  useLayoutEffect(() => {
    if (contentContext.viewport && contentContext.isPositioned) {
      ...
      // @ts-expect-error: Function declarations are not allowed inside blocks in strict mode when targeting 'ES3' or 'ES5'
