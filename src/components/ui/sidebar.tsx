  ...
  return (
    <>
      <motion.div
        className={cn(
          SIDEBAR_WIDTH.OPEN,
          'h-full p-4 hidden  md:flex md:flex-col bg-white/70 flex-shrink-0 dark:bg-gray-950/70 opacity-100',
          className,
        )}
        animate={{
          width: animate
...
export function MobileSidebar({ className, children, ...props }: TMobileSidebar) {
  ...
  return (
    <>
      <div
        className={cn(
          'h-10 p-4 flex flex-row md:hidden  items-center justify-between bg-white/70 w-full dark:bg-gray-950/70',
        )}
        {...props}
      >
        <div className='z-20 flex justify-end w-full'>
          <MenuIcon className='text-gray-900 dark:text-gray-50' onClick={() => setOpen(!open)} />
        </div>
        <AnimatePresence>
          {open && (
            <motion.div
              ...
              className={cn(
                'fixed h-full w-full inset-0 bg-white/70 p-10 z-[100] flex flex-col justify-between dark:bg-gray-950/70',
                className,
              )}
            >
              <div
                className='absolute z-50 right-10 top-10 text-gray-900 dark:text-gray-50'
  ...
  return (
    ...
    >
      ...
      <motion.span
        ...
        className='text-gray-900 text-sm group-hover/sidebar:translate-x-1 transition duration-150 whitespace-pre inline-block !p-0 !m-0 dark:text-gray-50'
      >
        {link.label}
      </motion.span>
    </Link>
