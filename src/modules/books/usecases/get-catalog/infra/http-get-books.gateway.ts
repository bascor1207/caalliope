...
export class HttpGetBooksGateway implements ConnectorToGetBooks {
    ...
    async getBooks(): Promise<Partial<BooksModel.Book>[]> {
        const { data } = await axiosInstance.get('/book');
        return this.createReturnPayload(data.data);
    }

    async getBooksByAuthor(value: string): Promise<Partial<BooksModel.Book>[]> {
        const { data } = await axiosInstance.get(`/author/${value}`);
        return this.createReturnPayload(data.data);
    }

    async getBooksByGenre(value: string): Promise<Partial<BooksModel.Book>[]> {
        const { data } = await axiosInstance.get(`/genre/${value}`);
        return this.createReturnPayload(data.data);
    }

    async getBooksByName(value: string): Promise<Partial<BooksModel.Book>[]> {
        //TODO CREATE BY NAME METHOD IN BACK
        const { data } = await axiosInstance.get(`/genre/${value}`);
        return this.createReturnPayload(data.data);
    }

    private createReturnPayload(data: BooksModel.GetBooksReturn[]): Partial<BooksModel.Book>[] {
